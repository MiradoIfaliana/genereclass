...
{ 
    ...
    public GenereClass(String extension, String topath, String packageclass, String[] typeforgenere, String databasename,String databasetype,String pathtemplate,String pathClassvariable) {
  ...
  //String nametable, String columnname, String columntype, String columntechtype, String libimport
    public Vector<DetailTable> infotable(DatabaseMetaData metaData,String tablename)throws Exception{
            List<String> pkname=getPrimaryKeys(metaData, tablename);
            List<String[]> fkname=getForeignKeys(metaData,tablename);    
            ...
            while (columns.next()) {
                ...
                detailTable=new DetailTable(tablename.substring(0,1).toUpperCase()+tablename.substring(1,tablename.length()),columnName+"", columnType+"", typeAndLib[0], typeAndLib[1],false,false,null,null);
                //si c'est un pk
                for(int i=0;i<pkname.size();i++){
                    if(columnName.equals(pkname.get(i))==true){
                        detailTable.setEstPK(true);
                        detailTable.setEstFK(false);
                        i=pkname.size();//fin boucle
                    }
                }
                //si c'est un fk
                for(int i=0;i<fkname.size();i++){
                    if(columnName.equals(fkname.get(i)[2])==true){
                        detailTable.setEstFK(true);
                        detailTable.setTabOriginFk(fkname.get(i)[0]);
                        detailTable.setColFromOrigin(fkname.get(i)[1]);
    ...
    public String getCreateTemplate(String template,DetailClass detailClass)throws Exception{
        ...
        for(int i=0;i<fields.length;i++){
            ...
            objtemp=fields[i].get(detailClass);
            if(objtemp!=null){
                if(objtemp instanceof String[]){
                    stemps=(String[])objtemp;
                    stemp="";
                    for(int j=0;j<stemps.length;j++){
                        stemp=stemp+""+stemps[j];
                    }
                    contenu=contenu.replaceAll( "#"+fields[i].getName()+"#", stemp);
                }else{
                    contenu=contenu.replaceAll( "#"+fields[i].getName()+"#", fields[i].get(detailClass).toString());
                }
            }else { contenu=contenu.replaceAll( "#"+fields[i].getName()+"#",""); }
...
//rehefa -1 le izy de tsy atao fotsiny
    ...
    public String getCodeStringClassByTablename(DatabaseMetaData metaData,String tablename)throws Exception{
        ...
        String extendsTemplate=getStringIn(template, "<extends>", "</extends>");
        String extends_="";
        String getsetfield="";
        DetailClass detailClass=getDetailClassByExtension();
        for(int i=0;i<vDetailTables.size();i++){
            ...
            if(detailTable.isEstPK()==true){
                field+=detailClass.getAnnotationPk()+"\n"+fieldTemplate.replaceAll("<typefield>",detailTable.getColumntechtype()).replaceAll("<namefield>", detailTable.getColumnname())+"\n";
            }else{
                field+=fieldTemplate.replaceAll("<typefield>",detailTable.getColumntechtype()).replaceAll("<namefield>", detailTable.getColumnname())+"\n";
            }
            if(getsetfieldTemplate.replaceAll(" ","").compareTo("")!=0){
        ...
        String [] strstemp=detailClass.getLibraries();
        for(int i=0;i<strstemp.length;i++){
                library+=libraryTemplate.replaceAll("<lib>", strstemp[i])+"\n";
        }
        //System.out.println("2"+library);
        strstemp=detailClass.getExtendsclass();
        for(int i=0;i<strstemp.length;i++){
                extends_+=extendsTemplate.replaceAll("<extendsclass>", strstemp[i])+" ";
        }
        ...
        indexdebut=template.indexOf("<extends>");
        indexfin=template.indexOf("</extends>"); //remplacena le eo ampovoany
        if(indexdebut!=-1 && indexfin!=-1){ 
            template=template.substring(0,indexdebut)+extends_+template.substring(indexfin+"</extends>".length(), template.length());
        }

        template=template.replaceAll("<package>", this.packageclass);//
        template=template.replaceAll("<Nameclass>", classname);
        template=template.replaceAll("<nameclass>", tablename);//
        return template;
    } 
    ...
    public void createclassbytablename(DatabaseMetaData metaData,String tablename)throws Exception{
        ...
        if(this.topath.substring(topath.length()-1).compareTo("/")!=0){ 
            this.topath+="/"; 
        }
    ...
    public void createclasses(Connection connection,String tablename)throws Exception{
        if(tablename.compareTo("*")==0){
            createclassOfdatabase(connection);
        }else{
            DatabaseMetaData metaData=connection.getMetaData();
            createclassbytablename(metaData,tablename);
        }
    }
